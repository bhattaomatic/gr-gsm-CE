...
class airprobe_bladerf(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, fc=939.4e6*0+948e6*0+945.6e6, gain=30, ppm=0, samp_rate=2000000/2, shiftoff=400e3):
        ...
        self.msg_connect((self.gsm_receiver_0, 'CIR'), (self.blocks_pdu_to_tagged_stream_0, 'pdus'))    
        self.msg_connect((self.gsm_receiver_0, 'measurements'), (self.gsm_clock_offset_control_0, 'measurements'))    
        self.msg_connect((self.gsm_receiver_0, 'CIR'), (self.zeromq_pub_msg_sink_0, 'in'))    
        self.connect((self.blocks_complex_to_mag_0, 0), (self.qtgui_time_sink_x_0, 0))    
        self.connect((self.blocks_pdu_to_tagged_stream_0, 0), (self.blocks_complex_to_mag_0, 0))    
...
def argument_parser():
    ...
    parser.add_option(
        "-f", "--fc", dest="fc", type="eng_float", default=eng_notation.num_to_str(939.4e6*0+948e6*0+945.6e6),
    ...
    parser.add_option(
        "-s", "--samp-rate", dest="samp_rate", type="eng_float", default=eng_notation.num_to_str(2000000/2),
...
def main(top_block_cls=airprobe_bladerf, options=None):
    if options is None:
    ...
    tb = top_block_cls(fc=options.fc, gain=options.gain, ppm=options.ppm, samp_rate=options.samp_rate, shiftoff=options.shiftoff)
    tb.start()
    tb.show()
