...
# Generated: Mon Mar 21 09:45:02 2016
##################################################
...
class airprobe_bladerf(gr.top_block, Qt.QWidget):
    ...
    def set_shiftoff(self, shiftoff):
        self.shiftoff = shiftoff
        self.blocks_rotator_cc_0.set_phase_inc(-2*pi*self.shiftoff/self.samp_rate)
        self.osmosdr_source_0.set_center_freq(self.fc_slider-self.shiftoff, 0)
        self.osmosdr_source_0.set_bandwidth(250e3+abs(self.shiftoff), 0)
    ...
    def set_ppm_slider(self, ppm_slider):
        ...
        self.osmosdr_source_0.set_freq_corr(self.ppm_slider, 0)
    ...
    def set_g_slider(self, g_slider):
        ...
        self.osmosdr_source_0.set_gain(self.g_slider, 0)
...
if __name__ == '__main__':
    ...
    parser.add_option("-f", "--fc", dest="fc", type="eng_float", default=eng_notation.num_to_str(939.4e6*0+948e6),
        help="Set fc [default=%default]")
    (options, args) = parser.parse_args()
    if(StrictVersion(Qt.qVersion()) >= StrictVersion("4.5.0")):
        Qt.QApplication.setGraphicsSystem(gr.prefs().get_string('qtgui','style','raster'))
    qapp = Qt.QApplication(sys.argv)
    tb = airprobe_bladerf(gain=options.gain, ppm=options.ppm, samp_rate=options.samp_rate, shiftoff=options.shiftoff, fc=options.fc)
    tb.start()
    tb.show()
    def quitting():
        tb.stop()
